...
Crons are tuned to reduce Vercel invocations while preserving behavior. When `CRON_SECRET` is set in Vercel, cron endpoints require `Authorization: Bearer <CRON_SECRET>` (Vercel injects this automatically).
...
### Technical Details
- Cache TTL during locked slate: **60 seconds** (balances responsiveness and Vercel cost)
